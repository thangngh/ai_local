...
def test_developer_sprint_plan_cuts_phases_into_twenty_one_sprints() -> None:
    plan = load_developer_sprint_plan(ROOT / "configs" / "developer_sprints.yaml")

    assert len(plan.sprints) == 21
    assert plan.sprints[0].phase == "phase_1_core_loop"
    assert plan.sprints[-1].phase == "phase_8_skill_runtime"
